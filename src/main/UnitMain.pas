...
type
  TFormMain = class(TForm)
    ...
    procedure cbBLOKSoundsClick(Sender: TObject);
  private
...
var
  ...
  WagonsAmount:                Byte;       // ���������� ������� �� ����� settings.ini
  // ----------------------------------------------------- //
  ...
  BV_Paketnik,       PrevBV_Paketnik: Integer;      // �������� ��
  Voltage,           PrevVoltage:     Single;       // ���������� �� ����������� ��7
  CameraX,           PrevCameraX:     WORD;         // ���������� ��� ����������� ��������� ������ � ������
  CameraX_Steam,     PrevCameraX_Steam:Double;      // ���������� ��� ����������� ��������� ������ � ������ ��� Steam ������
  ...
  PrevPRS:                            Integer;
  PRSUzSamplesCount:                  Integer;      // ���-�� ������� .mp3 � ����� PRS (��)
  PRSRZDSamplesCount:                 Integer;      // ���-�� ������� .mp3 � ����� PRS (���)
  ...
  WagsNum,           PrevWagsNum:     Byte;       // ���-�� ������� � ����� ������� (�� ���) WagonsAmount - �� settings.ini
  ...
  Brake_slipp_Volume:          Single;
  CompoundPercent:             Double;  // ��� ������� 0-����� 1-����������
  // ------------------------------------------------------ //
  // ******* ����� ******* //
...
try
  ...
  if (isConnectedMemory=True) then begin
     ...
     if cbBLOKSounds.Checked=True then begin   // БЛОК
   // Нажатие РБ и РБС
   if RB<>PrevRB then begin
      if RB=1 then begin RBF:=PChar('TWS/BLOK/block_pick.wav'); isPlayRB:=False; end;
      if RB=0 then begin RBF:=PChar('TWS/BLOK/block_pick.wav'); isPlayRB:=False; end;
   end;

   if RBS<>PrevRBS then begin
      if RBS=1 then begin RBF:=PChar('TWS/BLOK/block_pick.wav'); isPlayRB:=False; end;
      if RBS=0 then begin RBF:=PChar('TWS/BLOK/block_pick.wav'); isPlayRB:=False; end;
   end;
   
   // Контроль ограничивающей скорости (аналогично КЛУБ-У)
   if(OgrSpeed-Speed<=3) and (isPlayOgrSpKlub=0) and (OgrSpeed<>0) and (Svetofor<>0) then isPlayOgrSpKlub:=1;
   if((OgrSpeed-Speed>3) and (isPlayOgrSpKlub=-1)) or ((OgrSpeed=0) and (isPlayOgrSpKlub=-1)) then begin
      BASS_ChannelStop(Ogr_Speed_KLUB); BASS_StreamFree(Ogr_Speed_KLUB); isPlayOgrSpKlub:=0; 
   end;
...
// === ������� �� ������� "���� ������ ������� ��� ����������" === //
procedure TFormMain.cbBLOKSoundsClick(Sender: TObject);
begin
   if cbBLOKSounds.Checked=True then begin
